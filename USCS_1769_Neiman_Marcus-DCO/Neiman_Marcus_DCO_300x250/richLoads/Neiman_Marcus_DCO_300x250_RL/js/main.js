var adWidth = 300, adHeight = 250,  feedSuccess = false, variables, currentSlide = 0, productCount = 0;
var log = false;
...
function feedLoaded(feedArray, feedUrl) {
    ...
    for(var i = 0; i < productCount; i++) {
        ...
        listItem.appendChild(createCarouselText('brand', feedArray[i].brand));
        listItem.appendChild(createCarouselText('productName', feedArray[i].powerfeeds_name));
        // listItem.appendChild(createCarouselText('productDescription', feedArray[i].description));
        listItem.appendChild(createCarouselText('price', '$' + feedArray[i].powerfeeds_price));
        var frame2Cta = createCarouselText('cta2', variables.ctaTxt);
...
function createCarouselText(className, content) {
...
function preloadImage(feedImages) {
    var imageCount = 3 + feedImages.length, imageLoaded = 0;
...
function animationStart() {
    ...
    TweenMax.to(bg, .5, {opacity: 1, delay: 0});
    TweenMax.to(f1Logo, .5, {x: 0, delay: .5});
    TweenMax.to(f1cta, .5, {x: 0, delay: .5});
    if(!feedSuccess) return;

    TweenMax.set(frame2, {x: 300, opacity: 1});
    TweenMax.to(frame1, .5, {x: -300, delay: 3});

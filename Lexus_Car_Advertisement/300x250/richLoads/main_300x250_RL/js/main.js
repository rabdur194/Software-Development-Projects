...
function frameOne() {
    ...
    gsap.set(['#F1_bg'], { scale: 1.03, x: 5, z: 0.1, rotationZ: 0.01, force3D: true });
    gsap.set(['#F1_logo', '#F1_title'], { opacity: 1 });

    gsap.to(['#F1_bg'], { duration: 4, scale: 1.2, x: 5, y: -1, z: 0.1, rotationZ: 0.01, force3D: true });

    var tl = gsap.timeline({ delay: 0.3 });
    tl
        .fromTo(['#GL_line'], { scaleY: 0 }, { duration: 0.5, scaleY: 1, ease: 'power4.in', z: 0.1, rotationZ: 0.01, force3D: true })
        .from(['#F1_copy'], { duration: 0.85, x: -200, opacity: 0, ease: "power2.inOut" })
...
function frameTwo() {
    ...
    gsap.set(['#F2_bg'], { scale: 1.03, x: 5, z: 0.1, rotationZ: 0.01, force3D: true });

    gsap.to(['#F2_bg'], { duration: 4, scale: 1.2, x: 5, y: -1, z: 0.1, rotationZ: 0.01, force3D: true });

    gsap.from(['#F2_copy'], { delay: 0.3, duration: 0.85, x: -200, opacity: 0, ease: "power2.inOut" })

    var frameDelay = 3.5;
    gsap.to(['#F1_copy', '.F2', '#GL_line'], { opacity: 0, delay: frameDelay });
...
function endFrame() {
    ...
    gsap.from(['#EF_bg'], { duration: 3, scale: 1.2, x: 0, y: 0, z: 0.1, rotationZ: 0.01, force3D: true });

    gsap.to(['#F1_logo'], { duration: 0.4, x: -155 });
    gsap.to(['#F1_title'], { duration: 0.4, opacity: 0 });

    gsap.from(['#EF_copy1'], { duration: 0.85, x: -200, opacity: 0, ease: "power2.inOut" })
    gsap.from(['#EF_copy2'], { duration: 0.85, x: -200, delay: 1, opacity: 0, ease: "power2.inOut" })

    var tl = gsap.timeline({ delay: 0.3 });
    tl
        .fromTo(['#EF_line'], { scaleY: 0 }, { duration: 0.5, scaleY: 0.8, ease: 'power4.in', z: 0.1, rotationZ: 0.01, force3D: true })
...
function ctaAnim() {
    ...
    tl.from(['#EF_cta'], { duration: 0.5, opacity: 0, ease: 'power2.inOut' }, 'cta')
}
